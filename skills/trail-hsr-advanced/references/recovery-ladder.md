...
3. 先做 request-status / state 回读，再决定要恢复启动、窗口还是 session；对应命令可用 `trail daemon request-status --request-id <id>` / `trail --format yaml state dump --session <id>`，不要跳过状态确认直接连发动作。

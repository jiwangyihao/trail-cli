...
- 从投资环境页内部切入时，进入无人值守模式，不再继续追问，而是根据当前投资环境、`待收集=1` 与其他已知限制自动选攻略；不能假设 portal 卡片自带热度、版本或互动数据，需要这些动态信号时先显式调用 `guide.list.cw` / `guide.fetch.cw`。

## Command Surface

- `guide list cw`：先筛候选攻略，适合按投资环境、羁绊或角色方向缩小范围。
- `guide fetch cw`：在候选里读取完整攻略内容，确认标签、阵容、运营思路与当前目标是否一致。
- `guide.fetch.cw --select`：在确认候选后，把完整攻略写入当前 session，作为当前攻略；它不创建额外追踪产物，这是从选攻略切回入口前的关键一步。
...
- 如果这是从投资环境页内部切入的无人值守模式，就不再继续追问，而是按当前投资环境、`待收集=1` 与其他已知限制自动选攻略；需要热度、版本或互动数据时，先显式调用 `guide.list.cw` / `guide.fetch.cw`。

...
1. 如果还不确定窗口是否可操作，先运行 `trail window attach --window-title "崩坏：星穹铁道"`
2. 运行 `trail session create`
3. 从返回的 `data.session_id` 记录本局会话 ID
4. 把后续所有场景命令都显式带上 `--session <id>`
5. 需要进入货币战争时，切换到 `trail-cw`

...
def _normalize_strategy_ocr_card_inputs(values: object) -> list[dict[str, object]]:
    ...
    for lane_index, lane_pieces in enumerate(lanes):
        lane_text = _build_strategy_lane_text(lane_pieces)
        lane_text = _strip_strategy_marker_text(lane_text)
        refresh_count = _extract_strategy_refresh_count(lane_text)
        lane_body_text = _strip_strategy_refresh_text(lane_text)
        if not _normalize_strategy_text(lane_body_text):
            continue
        normalized.append(
            {
                "card_idx": lane_index + 1,
                "strategy_title": lane_body_text,
